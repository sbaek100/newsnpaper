...
## 단계 3.5 — 🔴 수집 품질 게이트 (SPEC-012)

| # | 작업 | 통과선 | 근거 |
|---|---|---|---|
| 3.5.1 | 6개 축 전량 측정 | — | PRD-03 §4.5.1 |
| 3.5.2 | 오병합 검수 (Claude) | **0건** | FR-63~65 |
| 3.5.3 | 관련성 표본 50건 판정 (Claude) | **≥ 85%** | FR-68 |
| 3.5.4 | 섹션 추출 | **≥ 60% / 95%** | FR-71 |

> **미달 축이 있으면 원인을 고친 뒤 재측정한다** (FR-70). 통과 전에 단계 4로 가지 않는다.
> 축 4가 50% 미만이면 "3섹션 번역" 결정 자체를 재검토한다.
...
## 단계 4.5 — 🔴 번역 품질 게이트 (SPEC-011)

| # | 작업 | 통과선 | 근거 |
|---|---|---|---|
| 4.5.1 | 실데이터 평가셋 30건 구축 | 논문 5편 3섹션 온전 | PRD-03 FR-55~57 |
| 4.5.2 | EXAONE vs Qwen A/B 실행 | — | FR-39a |
| 4.5.3 | Claude 1차 채점 (4항목) | **평균 ≥ 4.0** | FR-58 |
| 4.5.4 | 사용자 블라인드 A/B 10건 | 사용자 판정 | FR-59 |

> 둘 다 4.0 미달이면 **더 큰 모델 또는 번역 특화 모델을 추가 후보로** 검토한다 (FR-42a).
> 미달인 채로 진행하지 않는다.
...
**품질 게이트 2개가 순서를 강제한다:**
- 단계 3.5(수집 품질)를 통과해야 단계 4로 간다
- 단계 4.5(번역 품질)를 통과해야 단계 5로 간다

두 게이트 모두 **Claude 채점 + 사용자 최종 판정**이 들어간다. 자동 통과는 없다.
