...
## 4.2 🔴 자동 이상 탐지 (PRD-03 §8.7.1)

번역 결과를 DB에 쓰기 **전에** 검사한다. 평가 도구(`benchmarks/quality_ab.py`)의
구현을 그대로 베끼지 마라 — 반복 탐지에 오탐이 있는 상태다 (PRD-03 §8.7.2).

| ID | 요구사항 |
|---|---|
| T-12a | 치명 3종(`cjk_contamination`·`cyrillic_contamination`·`mojibake`)을 검사한다. 한자 1자라도 있으면 실패다 |
| T-12b | 치명이 나오면 **DB에 쓰지 말고 재시도**한다 (FR-61a) |
| T-12c | 반복 탐지는 **"100자 이상 2회" 또는 "30자 이상 3회"** 의 OR로 한다 (FR-61d). 30자 2회로 하지 마라 — 정상 학술 문장에서 오탐이 난다 |
| T-12d | 경고 4종은 기록만 하고 노출을 막지 않는다 (FR-61b) |
| T-12e | 2패스 검증 출력에 치명이 나오면 **초벌을 채택**한다 (FR-76) |
